...
<%
	...
	//Criteria 객체를 생성하여 검색 조건과 페이지 번호를 담음
	...
	//getTotalCnt 메서드를 사용하여 총 게시물 수 가져옴
...
<%@include file="../../Session/Link.jsp" %>
	...
	<!-- el에서 사용하기 위해 변수를 페이지 영역에 저장 -->
     ...
     <c:if test="${not res }">
     	...
     	<tr align= "center">
			<th> ${board.num}</td> <!--게시물 번호-->
			<td align= "left">
				<a href="View.jsp?num=${board.num}&pageNo=<%=criteria.getPageNo()%>">${board.title}</a></td> <!--제목(+ 하이퍼링크)-->
			<td align= "center">${board.id} </td> <!--작성자 아이디-->
			<td align= "center">${board.visicount} </td> <!--조회수-->
...
%>
	...
	<table border="1" width="90%">
		<tr>
			<td align="center">
				<%@include file="/Paging/PageNav.jsp" %>
			</td>
		</tr>
	</table>
	<!-- 페이지 블럭 생성 끝 -->
